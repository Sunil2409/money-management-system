/**
 * Money Manager — Frontend Application (httpOnly Cookie Authentication)
 * Data Flow: User Action → JS Event → fetch() with credentials → Django DRF → Database → JSON → DOM Update
 * 
 * Security: All authentication handled via httpOnly cookies (automatic, JS cannot access).
 * No more localStorage tokens to steal via XSS.
 */

// Use relative URL to go through nginx proxy
// This ensures same-origin requests and proper cookie handling
const API_BASE = 'https://mm-sunil2409-backend.onrender.com/api';
...
// ── Auth Guard ──
/**
 * Check if user is authenticated by attempting to fetch /me
 * If not authenticated, redirect to login page.
 * Uses httpOnly cookies automatically (no token handling needed).
 */
...
// ── Authenticated Fetch ──
/**
 * Fetch wrapper that automatically includes httpOnly cookies.
 * Cookies are sent automatically by the browser with credentials: 'include'.
 * 
 * If server returns 401 (token expired), attempt to refresh the token using
 * the refresh cookie, then retry the original request.
 */
 ...
 * Server will set a new access token cookie in response.
 */
async function refreshToken() {
  try {
    const res = await fetch(`${API_BASE}/auth/token/refresh/`, {
      method: 'POST',
      credentials: 'include', // Send refresh token cookie, receive new access token
 ...
 * Server will clear auth cookies in response.
 */
...
async function loadUserProfile() {
  try {
    ...
    if (!res) {
      // authFetch will handle logout if needed
      return;
    }
    ...
    if (res.ok) {
      ...
      // Non-200 response, assume auth failed
      console.error('Profile load failed:', res.status);
      logout();
    }
  } catch (err) {
    console.error('Profile load error:', err);
    logout(); // Force logout on critical network error
  }
}
...
function initTheme() {
  ...
  document.getElementById('themeToggle').addEventListener('click', () => {
    ...
    localStorage.setItem('mm-theme', next); // Theme preference is safe in localStorage
  });
}
...
async function handleSubmit(e) {
  ...
  try {
    const url = currentEditId 
      ? `${API_BASE}/transactions/${currentEditId}/` 
    ...
    if (!res || !res.ok) {
      const err = res ? await res.json() : { detail: 'Request failed' };
      throw new Error(Object.values(err).flat().join(', ') || 'Failed to save');
    }
    ...
    setTimeout(() => {
      showSection(currentEditId ? 'transactions' : 'dashboard');
    }, 500);
  } catch (err) {
