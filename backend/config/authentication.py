...
Extends DRF's JWTAuthentication to extract JWT tokens from httpOnly
cookies instead of (or in addition to) the Authorization header.
...
Fallback: Also supports Bearer token in Authorization header
for API testing/clients.
"""
...
class CookieJWTAuthentication(JWTAuthentication):
    ...
    def authenticate(self, request):
        ...
        # If no token found, return None (let other auth classes try)
        if not access_token:
            return None
